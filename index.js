...
function indekseGoreFenomen(fenomenlerDizi,index) {
 
  const result =`${index}. indekste bulunan fenomen: ${fenomenlerDizi[index].profile}`;
  return result
}
...
function fenomenSil(fDizi,index) {
  let yDizi=[...fDizi];
  yDizi.splice(index,1);
  return yDizi;
...
function enFenomenler(fDizi) {
  let result= [];
  for (let i=0 ; i< fDizi.length;i++){
    if(fDizi[i].followers>100000000){

      result.push(fDizi[i].profile);
    }
...
function fenomenGonderimSayisi(fDizi,name){
  for (let i=0 ;i < fDizi.length ;i++){
    if(fDizi[i].profile===name){
      return fDizi[i].posts;
    }
...
function platformaGoreCokGonderiYapanFenomen(dizi, platform) {
  ...
  for (let i = 0; i < dizi.length; i++) {
    if (dizi[i].posts>max && dizi[i].platform==platform) {
      max=dizi[i].posts;
      result=dizi[i].profile; 
    }
  }
  return result;
  }
